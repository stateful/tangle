...
function forWebviews(namespace, defaultValue, wvProviders) {
  return merge(...wvProviders.map(wv => wv.webview)).pipe(take(wvProviders.length), toArray(), map(wvs => {
    return wvs.map(wv => {
      const provider = {
        onMessage: listener => {
...
class Client {
  ...
  on(eventName, fn) {
    this.events.pipe(mergeMap(events => from(Object.entries(events))), filter(([k]) => k.toLowerCase().indexOf(eventName.toLowerCase()) >= 0), map(([_, v]) => v)).subscribe(fn);
  }
  ...
  subscribe(key, fn, distinctUntilChanged = false) {
    if (distinctUntilChanged) {
      return this.transient.pipe(distinctUntilKeyChanged(key), pluck(key), filter(val => val !== undefined)).subscribe(fn);
    } else {
      return this.transient.pipe(pluck(key), filter(val => val !== undefined)).subscribe(fn);
    }
  }
  ...
  register() {
    ...
    const transientGrouped$ = merge(inGrouped$.pipe(pluck("transient")), outGrouped$.pipe(pluck("transient")));
    const inEvent$ = inGrouped$.pipe(pluck("event"));
    inEvent$.subscribe(event => {
      this._events.next(event);
    });
    const transient$ = merge(providers).pipe(this.fromProviders(), mergeMap(provider => {
      return transientGrouped$.pipe(scan(this.fold, this.defaultValue), throttleTime(20), mergeMap(transient => {
        const namespaced = {};
    ...
    event$.pipe(withLatestFrom(transient$), map(([event, transient]) => {
      return { ...transient,
        ...event
      };
    }), map(combo => {
  ...
  dedupe() {
    return source => {
      return source.pipe(startWith(this.defaultValue), pairwise(), filter(([prev, curr]) => JSON.stringify(prev) !== JSON.stringify(curr)), map(([_, curr]) => curr));
    };
  }
...
export { Bus, Client, forDOM, forWebviews, wrapPanel };
