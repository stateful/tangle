...
class Client {
    constructor(namespace, providers, defaultValue, _isBus = false) {
        this.namespace = namespace;
        this.providers = providers;
        this.defaultValue = defaultValue;
        this._isBus = _isBus;
        this.id = this._isBus ? 'bus' : Math.random().toString(36).substring(2);
        this._notifer = new Subject();
        this._eventMap = new Map();
        this._context = { clients: new Map() };
        this._outbound = new Subject();
        this._inbound = new BehaviorSubject({ transient: this.defaultValue, context: this._context });
        this._events = new Subject();
        this._context.clients.set(this.id, this._isBus);
        this.context = this._registerContext();
        this._transient = this._register();
        if (this._isBus) {
            this.context.pipe(debounceTime(100), withLatestFrom(this.transient)).subscribe(([, previous]) => this.broadcast(previous || this.defaultValue));
        }
    }
    get events() {
    ...
    _registerContext() {
        const context$ = this._inbound.pipe(map(inbound => inbound === null || inbound === void 0 ? void 0 : inbound.context), filter(Boolean), scan((acc, curr) => {
            if (Array.isArray(curr === null || curr === void 0 ? void 0 : curr.clients)) {
                const _curr = { clients: new Map(curr.clients) };
    ...
    listen(eventName, fn) {
        ...
        return this.events.pipe(map(event => event === null || event === void 0 ? void 0 : event.transient), map(transient => transient === null || transient === void 0 ? void 0 : transient[eventName]), filter(Boolean)).subscribe(fn);
    }
    emit(eventName, payload) {
        this._outbound.next({ event: { [eventName]: payload } });
    }
    on(eventName, fn) {
    ...
    _registerEvent(eventName, fn, isOnce = false) {
        const events = this._eventMap.get(eventName) || [];
        const index = typeof eventName === 'string'
            ? eventName.toLocaleLowerCase()
            : eventName.toString().toLowerCase();
        const obs = this.events
            .pipe(map(e => e === null || e === void 0 ? void 0 : e.event), filter(Boolean), mergeMap((events) => from(Object.entries(events))), filter(([k]) => k.toLowerCase().indexOf(index) >= 0), map(([, v]) => v), (source) => {
            if (!isOnce) {
                return source;
            }
    ...
    _register() {
        ...
        const transientCombined$ = merge(this._inbound.pipe(map(inbound => inbound === null || inbound === void 0 ? void 0 : inbound.transient)), this._outbound.pipe(map(outbound => outbound === null || outbound === void 0 ? void 0 : outbound.transient)));
        const transient$ = merge(providers).pipe(this._fromProviders(), mergeMap(() => {
            return transientCombined$.pipe(scan(this._fold, this.defaultValue), throttleTime(20), mergeMap((transient) => {
                const namespaced = {};
                namespaced[this.namespace] = transient;
                return of(transient);
            }));
        }), bufferCount(this.providers.length), map((transients) => {
        ...
        event$
            .pipe(withLatestFrom(transient$), map(([event, transient]) => {
            if (event.event) {
    ...
    _fromProviders() {
        return (source) => {
            return source.pipe(map((provider) => {
...
class Bus extends Client {
    constructor(namespace, providers, defaultValue) {
        super(namespace, providers, defaultValue, true);
    }
    whenReady() {
        return firstValueFrom(this.context.pipe(bufferCount(this.providers.length), map(() => this._context)));
    }
}
